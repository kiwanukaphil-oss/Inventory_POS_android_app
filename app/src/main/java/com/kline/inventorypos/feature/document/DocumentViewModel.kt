...
data class DocumentUiState(val documents: List<BusinessDocument> = emptyList(), val detail: BusinessDocument? = null, val type: String? = null, val status: String? = null, val query: String = "", val canCreate: Boolean = false, val canEdit: Boolean = false, val canVoid: Boolean = false, val loading: Boolean = false, val working: Boolean = false, val uncertain: Boolean = false, val error: String? = null, val message: String? = null)
class DocumentViewModel(private val repository: BusinessDocumentRepository) : ViewModel() {
    ...
    fun bindSession(session: PosSession) { val newKey = "${session.user.id}:${session.branch.id}"; if (newKey == key) return; key = newKey; state.value = DocumentUiState(canCreate = session.user.hasPermission("documents.create"), canEdit = session.user.hasPermission("documents.edit"), canVoid = session.user.hasPermission("documents.void")); refresh() }
    ...
    fun transition(id: String, status: String) = mutate("Status updated") { repository.transition(id, status) }; fun void(id: String, reason: String) = mutate("Document voided") { repository.void(id, reason) }; fun convert(id: String, method: String?, reference: String) = mutate("Document converted") { repository.convert(id, method, reference) }
    private fun mutate(message: String, block: suspend () -> BusinessDocument) { if (state.value.working || state.value.uncertain) return; viewModelScope.launch { state.update { it.copy(working = true, error = null) }; runCatching { block() }.onSuccess { doc -> state.update { s -> s.copy(working = false, detail = doc, documents = listOf(doc) + s.documents.filterNot { it.id == doc.id }, message = message) } }.onFailure { e -> state.update { it.copy(working = false, uncertain = e is DocumentMutationUncertainException, error = e.message) } } } }
